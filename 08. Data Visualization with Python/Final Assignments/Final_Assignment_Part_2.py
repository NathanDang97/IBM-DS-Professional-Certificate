...
# Run the following commmands to run the file on Terminal
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div(id='output-containter', className='chart-grid', style={'display': 'flex'}),
    ])
])
...
def update_output_container(selected_statistics, input_year):
    if selected_statistics == 'Recession Period Statistics':
        ...
        # use groupby to create relevant data for plotting
        exp_rec= recession_data.groupby(['Recession', 'Vehicle_Type'])['Automobile_Sales'].mean().reset_index() 
        R_chart3 = dcc.Graph(
        ...
        unempl_rec= recession_data.groupby(['unemployment_rate', 'Vehicle_Type'])['Automobile_Sales'].mean().reset_index() 
        R_chart4 = dcc.Graph(
                    figure=px.bar(unempl_rec,
                    x='unemployment_rate',
                    y='Automobile_Sales',
                    color='Vehicle_Type',
